...
def go_to_next_step(event):
    ...
    #     spreadsheet_id=redis_client.hget(user_id,'spreadsheet_id')
    ...
    if request_text=="redis_reset":
        ...
        line_api.reply_message(reply_token,get_step_2_message())

    # a_step_2
    elif current_scenario =="a_step_2":
        redis_client.hset(user_id,'category',request_text)
        # a_step_3_1
        redis_client.hset(user_id,'current_scenario','a_step_3_1')
        line_api.reply_message(reply_token,get_step_3_1_message())

    # a_step_3_1
    elif current_scenario =="a_step_3_1":
        show_loading_animation(event)
        # a_step_3_2
        redis_client.hset(user_id,'current_scenario','a_step_3_2')
        image_processing_a_step_3(event)
        is_food_package=redis_client.hget(user_id,'is_food_package').lower()=="true"
        if is_food_package is True:
            line_api.reply_message(reply_token,get_step_3_2_message(event))
        else:
            # a_step_3_1
            redis_client.hset(user_id,'current_scenario','a_step_3_1')
            line_api.reply_message(reply_token,get_step_3_1_message())
    ...
    # a_step_3_2
        ...
        if request_text=="OK":
            # a_step_4_1
            redis_client.hset(user_id,'current_scenario','a_step_4_1')
            line_api.reply_message(reply_token,get_step_4_1_message())
        elif request_text=="画像を送信し直す":
            # a_step_3_1
            redis_client.hset(user_id,'current_scenario','a_step_3_1')
            line_api.reply_message(reply_token,get_step_3_1_message())
        else: 
            redis_client.hset(user_id,'temp', request_text)
            # a_step_3_3
            redis_client.hset(user_id,'current_scenario','a_step_3_3')
            line_api.reply_message(reply_token,get_step_3_3_message())

    # a_step_3_3
    elif current_scenario =="a_step_3_3":
        temp=redis_client.hget(user_id,'temp')
        if temp=="メーカーを修正":
            redis_client.hset(user_id,'manufacturer',request_text)
        elif temp=="商品名を修正":
            redis_client.hset(user_id,'product_name',request_text)
        # a_step_3_2
        redis_client.hset(user_id,'current_scenario','a_step_3_2')
        line_api.reply_message(reply_token,get_step_3_2_message(event))
    ...
    # a_step_4_1
        ...
        show_loading_animation(event)
        # a_step_4_2
        redis_client.hset(user_id,'current_scenario','a_step_4_2')
        image_processing_a_step_4(event)
        is_ingredient_list=redis_client.hget(user_id,'is_ingredient_list').lower()=="true"
        if is_ingredient_list is True:
            line_api.reply_message(reply_token,get_step_4_2_message(event))
        else:
            # a_step_4_1
            redis_client.hset(user_id,'current_scenario','a_step_4_1')
            line_api.reply_message(reply_token,get_step_4_1_message())
    ...
    # a_step_4_2
        ...
        if request_text=="OK":
            # a_step_8
            redis_client.hset(user_id,'current_scenario','a_step_8')
            line_api.reply_message(reply_token,get_step_8_message(event))
        elif request_text=="画像を送信し直す":
            # a_step_4_1
            redis_client.hset(user_id,'current_scenario','a_step_4_1')
            line_api.reply_message(reply_token,get_step_4_1_message())
            ...
            redis_client.hset(user_id,'temp', request_text)
            # a_step_4_3
            redis_client.hset(user_id,'current_scenario','a_step_4_3')
            line_api.reply_message(reply_token,get_step_4_3_message())

    # a_step_4_3
    elif current_scenario =="a_step_4_3":
        temp=redis_client.hget(user_id,'temp')
        if temp=="〇〇g当たりを修正":
            redis_client.hset(user_id,'gram_per_unit',request_text)
        elif temp=="g以外の単位を修正":
            redis_client.hset(user_id,'measurement_unit',request_text)
        elif temp=="熱量を修正":
            redis_client.hset(user_id,'calories',request_text)
        elif temp=="たんぱく質を修正":
            redis_client.hset(user_id,'protein',request_text)
        elif temp=="脂質を修正":
            redis_client.hset(user_id,'fat',request_text)
        elif temp=="炭水化物を修正":
            redis_client.hset(user_id,'carbohydrates',request_text)
        elif temp=="食塩相当量を修正":
            redis_client.hset(user_id,'sodium',request_text)
        # a_step_4_2
        redis_client.hset(user_id,'current_scenario','a_step_4_2')
        line_api.reply_message(reply_token,get_step_4_2_message(event))
    ...
    # a_step_8
        ...
        if request_text=="Googleスプレッドシートに登録":
            redis_client.hset(user_id,'current_scenario','a_step_9')
            ...
            data = {key: (get_redis_value(redis_client, user_id, key) or "") for key in keys}
            ...
            line_api.reply_message(reply_token, get_step_9_message())
    ...
    # a_step_9
        ...
        if request_text=="続けて登録":
            ...
            # シナリオ終了
            line_api.reply_message(reply_token,get_scenario_end_message())
            redis_client.delete(user_id)
